...
//  - BloomFilter: basic bit-array bloom filter with k hash functions
//  - CompoundBloomFilter: an array of BloomFilter objects; insert into all, query = AND of queries
//  - Small experiment comparing false positive rate (FPR) of single BF vs compound (2) BFs
...
// We keep total memory equal.
void run_experiment() {
    cout << "Running small experiment comparing single BF vs 2-filter CBF (same total bits)\n";

    const size_t total_bits = 1 << 20; // 1M bits (~128 KB) — adjustable
    const size_t k = 7;                // number of hash functions per filter
    ...
    cout << "  filters in CBF = " << filters_count << " (per-filter bits = " << per_filter_bits << ")\n";
    ...
    cout << "  Single BF false positives: " << false_pos_single << " / " << n_test
         << "  => FPR = " << fpr_single << " %\n";
    cout << "  2-filter CBF false positives: " << false_pos_cbf << " / " << n_test
         << "  => FPR = " << fpr_cbf << " %\n";
    cout << "  2-filter CBF2 false positives: " << false_pos_cbf2 << " / " << n_test
         << "  => FPR = " << fpr_cbf2 << " %\n";
    ...
    // cout << "  (both configured to have same total bits in this experiment)\n";
}
...
int main() {
    ...
    cout << "Compound Bloom Filter (CBF) demo\n";
    cout << "-------------------------------\n\n";
