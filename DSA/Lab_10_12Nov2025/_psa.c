...
int compareEvents(const void *a, const void *b) {
    ...
    Event *e2 = (Event *)b;
    if (e1->x == e2->x)
        return e1->type - e2->type; // start comes before end
...
// Function to compute intersection point between two segments
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        // printf("Enter coordinates of segment %d (x1 y1 x2 y2): ", i + 1);
        // scanf("%lf %lf %lf %lf", &segments[i].p1.x, &segments[i].p1.y, &segments[i].p2.x, &segments[i].p2.y);
        // segments[i].id = i;

        // Ensure p1.x < p2.x for proper sorting
