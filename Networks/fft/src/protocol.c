...
-----------------------------------------------------------*/
void build_pkt_file_hdr(pkt_file_hdr_t *pkt, const char *filename,
                        uint16_t file_name_len, uint64_t file_size) {
  if (!pkt)
    return;
  ...
  pkt->_max_rec_size = htons(DEFAULT_RECORD_SIZE);
  pkt->_max_records_per_packet = htons(DEFAULT_RECORDS_PER_PACKET);
  pkt->_max_packets_per_blast = htons(DEFAULT_PACKETS_PER_BLAST);

  // File metadata
  pkt->file_size = htonll(file_size);
  pkt->filename_len = htons(file_name_len);
...
-----------------------------------------------------------*/
void build_pkt_file_hdr_ack(pkt_file_hdr_ack_t *pkt, uint8_t status,
  ...
  pkt->_max_rec_size = htons(max_rec_size);
  pkt->_max_records_per_packet = htons(max_records_per_packet);
  pkt->_max_packets_per_blast = htons(max_packets_per_blast);
}

/*-----------------------------------------------------------
  IS_BLAST_OVER Packet Builder
-----------------------------------------------------------*/
void build_pkt_is_blast_over(pkt_is_blast_over_t *pkt, uint32_t blast_id,
                             uint32_t n_packets) {
  memset(pkt, 0, sizeof(pkt_is_blast_over_t));
  pkt->type = PKT_IS_BLAST_OVER;
  pkt->blast_id = htonl(blast_id);
  pkt->n_packets = htonl(n_packets);
}

/*-----------------------------------------------------------
  REC_MISS_HDR Packet Builder
-----------------------------------------------------------*/
void build_pkt_rec_miss_hdr(pkt_rec_miss_hdr_t *pkt, uint8_t n_missing,
                            const uint8_t *missing_arr) {
  memset(pkt, 0, sizeof(pkt_rec_miss_hdr_t));
  pkt->type = PKT_REC_MISS_HDR;
  pkt->n_packets_missing = n_missing;
  memcpy(pkt->is_pkt_missing, missing_arr, DEFAULT_PACKETS_PER_BLAST);
}
...
void build_pkt_disconnect(pkt_disconnect_t *pkt, uint32_t reason_code) {
  ...
  pkt->reason_code = htonl(reason_code);
}
